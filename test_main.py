...
class TestBooksCollector():
    ...
    def test_set_book_genre_add_genre(self, collector, book_in_dictionaries):
    ...
    @pytest.mark.parametrize('book, genre',[[book_one, 'Триллер'],
                                            ['Санта', genre_one],
                                            ['Терминатор', 'Боевик']])
    def test_set_book_genre_genre_not_add(self, collector, book_with_genre_in_dictionaries, book, genre):
        collector.set_book_genre(book, genre)
        assert collector.books_genre == {book_one: genre_one}

    #3.0 тестируем вывод жанра книги по её имени
    def test_get_book_genre_get_genre(self, collector, book_with_genre_in_dictionaries):
    ...
    def test_add_book_in_favorites_add_book(sels, collector, book_in_dictionaries):
        collector.add_book_in_favorites(book_one)
        collector.add_book_in_favorites(book_one)
        assert len(collector.favorites) == 1

    #8.0 тестируем удаляение книги из избранного
    def test_delete_book_from_favorites_book_deleted(self, collector, book_in_favorites):
